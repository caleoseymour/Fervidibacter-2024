...
logmeanGHs = annotations[logmeandt,][protein_id %in% GHs,][order(-baseMean),]
...
ymax = (logmeanGHs[,baseMean] * 1.05) %>% max()
...
logmeanGHs[, angle := 45 ]
logmeanGHs[as.numeric(protein_id) > 110, angle := 90 ]

logmeanGHs[,lab := paste0(ifelse(label != '', dbCAN, ''), ifelse(multidomain, '🞶', ''))]

barplots = ggplot() + 
    geom_bar(data = logmeanGHs, aes(x=protein_id, y=baseMean, fill = GHs), stat = "identity", width=0.9, color='black', linewidth=0.25) +
    geom_text(data = logmeanGHs, aes(x = protein_id, y = baseMean + (ymax * 0.01), label = lab), angle = logmeanGHs[,angle], size = 6 * (5/14), hjust = 0, vjust = 0.5, lineheight = 0.5, family='Arial') +
...
pdf(paste0('output-transcriptome/barplot.pdf'), width = 7, height = 3, family='Arial')
    plot(barplots2)
dev.off()

svg(paste0('output-transcriptome/barplot.svg'), width = 7, height = 3, family='Arial')
    plot(barplots2)
dev.off()
...
figure = plot_grid(
    ...
    plot_grid(
        ...
        labels = c('c','d'), label_fontfamily = 'Arial'
        )
    , nrow = 3
    , rel_heights = c(1 + 1/6, 0.3, 1 + 1/6)
    , label_size = max_text_size
    , labels = c('b','',''), label_fontfamily = 'Arial'
)

svglite(paste0('output-transcriptome/transcriptome-figure-', format(Sys.Date(), "%m-%d-%Y"), '.svg'), width = 6.05*8/7, height = 5.09*8/7)
    plot(figure)
dev.off()

pdf(paste0('output-transcriptome/transcriptome-figure-', format(Sys.Date(), "%m-%d-%Y"), '.pdf'), width = 6.05*8/7, height = 5.09*8/7, family='Arial')
    plot(figure)
dev.off()